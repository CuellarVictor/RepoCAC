...
namespace AuditCAC.CoreCAC.Services.Controllers
{
    ...
    {
        private readonly ICoberturasRepository<CoberturaModel> _repository;

        private static readonly log4net.ILog _log = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        //Constructor.
        public CoberturasController(ICoberturasRepository<CoberturaModel> Repository)
        {
        ...
        public async Task<IActionResult> Coberturas([FromBody] InputsCoberturaDto inputsCoberturaDto)
        {
            try
            {
                var Data = await _repository.GetcacCoberturas(inputsCoberturaDto);
                return Ok(Data);
            }
            catch (Exception ex)
            {
