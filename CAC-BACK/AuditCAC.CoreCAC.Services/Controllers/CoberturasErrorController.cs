...
namespace AuditCAC.CoreCAC.Services.Controllers
{
    ...
    [Route("api/CoberturasError")]
    [ApiController]
    [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
    public class CoberturasErrorController : ControllerBase
    {
        private readonly ICoberturasErrorRepository<CoberturasErrorModel> _repository;

        private static readonly log4net.ILog _log = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        //Constructor.
        public CoberturasErrorController(ICoberturasErrorRepository<CoberturasErrorModel> Repository)
        {
        ...
        public async Task<IActionResult> CoberturasError([FromBody] InputsCoberturasErrorDto inputsCoberturasErrorDto)
        {
            try
            {
                var Data = await _repository.GetcacCoberturasError(inputsCoberturasErrorDto);
                return Ok(Data);
            }
            catch (Exception ex)
            {
